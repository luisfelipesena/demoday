...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!isOwner && !isAdmin && !isProfessor) {
      return NextResponse.json({ error: 'Você não tem permissão para ver esta submissão' }, { status: 403 })
    }
...
export async function DELETE(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!isOwner && !isAdmin) {
      return NextResponse.json({ error: 'Você não tem permissão para remover esta submissão' }, { status: 403 })
    }
    ...
    // Remover a submissão
    await db.delete(projectSubmissions).where(eq(projectSubmissions.id, submissionId))

    return NextResponse.json({ message: 'Submissão removida com sucesso' })
  } catch (error) {
    console.error('Erro ao remover submissão:', error)
    return NextResponse.json({ error: 'Erro ao remover submissão' }, { status: 500 })
  }
}
