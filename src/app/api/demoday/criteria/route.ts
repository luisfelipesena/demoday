...
export async function POST(req: NextRequest) {
  try {
    ...
    if (session.user.role !== 'admin') {
      return NextResponse.json({ error: 'Apenas administradores podem criar critérios' }, { status: 403 })
    }
    ...
    if (body.demodayId !== undefined && (body.registration !== undefined || body.evaluation !== undefined)) {
      ...
      if (!batchResult.success) {
        return NextResponse.json({ error: 'Dados inválidos', details: batchResult.error.format() }, { status: 400 })
      }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (session.user.role !== 'admin') {
      return NextResponse.json({ error: 'Apenas administradores podem remover critérios' }, { status: 403 })
    }
    ...
    if (!id || !type) {
      return NextResponse.json({ error: 'ID e tipo do critério são obrigatórios' }, { status: 400 })
    }

    if (type === 'registration') {
      await db.delete(registrationCriteria).where(eq(registrationCriteria.id, id))
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Admin check
    if (session.user.role !== 'admin') {
      return NextResponse.json({ error: 'Apenas administradores podem atualizar critérios' }, { status: 403 })
    }
    ...
    await db.transaction(async (tx: typeof db) => {
      ...
      await tx.delete(registrationCriteria).where(eq(registrationCriteria.demoday_id, demodayId))

      await tx.delete(evaluationCriteria).where(eq(evaluationCriteria.demoday_id, demodayId))
    ...
    return NextResponse.json({ error: 'Erro ao atualizar critérios' }, { status: 500 })
  }
}
